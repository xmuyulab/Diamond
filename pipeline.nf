...
====================================================================================
            A Multi-modal DIA Mass Spectrometry Data Processing Pipeline.             
                  https://github.com/xmuyulab/Diamond/pipeline.nf                        
====================================================================================
*/
...
def helpMessage() {
    ...
    The typical command for running the pipeline is as follows:
        ...
        Start the pipeline with the aim to build a spectral library:
           \$ nextflow run pipeline.nf --workdir [] --centroid [] --profile [] --fasta [] --windows [] --windowsNumber []  <Options_01> <functions>
        
        Start the pipeline with providing a ready-made spectral library:
    ...
    ====================
        ...
        --centroid              Deliver MS data in centroid mode. For example: --centriod "/PATH/TO/workdir/centroid/*.mzXML"

        --profile               Deliver MS data in profile mode. For example: --profile "/PATH/TO/workdir/profile/*.mzXML"

        --fasta                 Deliver the protein database. For example: --fasta "/PATH/TO/db.fasta"
    
        --windows               Deliver the winodws file. For example: --windows "/PATH/TO/win.tsv"
        ...
        --lib                   Deliver a ready-made spectral library. For example: "/PATH/TO/lib.TraML"
    ...
    ====================
        ...
        --openSWATH_paraNumber  Specify the number of parallel processing for openSWATH.
...
def Warnings() {
    if (params.skipLibGeneration == false) {
        ...
        if ((params.windowsNumber == null) || (params.windowsNumber != 100 && params.windowsNumber != 32 && params.windowsNumber != 80)) {
            println "\nError:\n     The DIA-Umpire pamameter file can not be confirmed! " + 
            "Please add --windowsNumber parameter in the command-line and ensure the number " +
            "meets the requirements!\n"
            exit 0
        }
        if (params.workdir && params.centroid && params.profile && params.fasta && params.windows && params.windowsNumber) {
            println "\nThe pipeline starts with the aim to build a spectral library! The process OpenSWATH_withInputLib " +
            "will be skipped!\n"
        }
    }
        ...
        if (params.workdir && params.profile && params.irt && params.lib && params.windows) {
            ...
            "since a ready-made spectral library is provided! \n"
        }
    }
}
...
process diau {
    ...
    echo true
    cache "deep"
    publishDir path : "${outdir}/diau-mgf" , pattern: "*diau.mgf"
    publishDir path : "${outdir}/diau-temp"
...
process mzML_part {
    ...
    echo true 
    cache "deep"
    publishDir path : "${outdir}/mzML/split"
    publishDir path : "${outdir}", pattern: "*.split"
...
process tandem_search {
    ...
    echo true 
    cache "deep"
    publishDir path : "${outdir}", pattern: "*.tandem.params"
    publishDir path : "${outdir}", pattern: "*.kscore.params"
    publishDir path : "${outdir}", pattern: "*.tandem"
    publishDir path : "${outdir}", pattern: "*.kscore"
